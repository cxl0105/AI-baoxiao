...
- **多级审批流（三级）**：员工提交 → 部门主管 + 财务【并行】都批完 → 总经理/管理员终审通过；支持撤销、驳回、加签、转办
...
- **平台级租户管理**：平台管理员按纳税号新增/删除企业，新增企业自动生成管理员/总经理/财务/部门经理四类账号（密码 123456），部门经理可按部门在成员管理中继续添加
...
| 角色 | 手机号 | 邮箱 |
|---|---|---|
| 平台管理员 | 13900000000 | platform@example.com |
| 系统管理员 | 13800000001 | admin@example.com |
| 总经理 | 13800000005 | gm@example.com |
| 财务专员 | 13800000002 | finance@example.com |
| 部门主管 | 13800000006 | manager@example.com |
| 普通员工 | 13800000003 | employee@example.com |
| 演示用户 | 13800000004 | demo@example.com |

> 审批链：员工提交 → 部门主管 + 财务（并行，双方都批完）→ 总经理/管理员终审通过。
>
> 平台管理员（13900000000）不属于任何企业，登录后进入「企业管理」后台，按纳税号新增/删除企业（租户）。
